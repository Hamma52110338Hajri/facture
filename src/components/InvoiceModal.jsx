...
class InvoiceModal extends React.Component {
        ...
        <Modal show={this.props.showModal} onHide={this.props.closeModal} size="lg" centered>
          <div id="invoiceCapture">
            <div className="d-flex flex-row justify-content-between align-items-start bg-light w-100 p-4">
              <div className="w-100">
                <h4 className="fw-bold my-2">{this.props.info.billFrom||'facture e-commerse vite'}</h4>
                <h6 className="fw-bold text-secondary mb-1">
                Facture #: {this.props.info.invoiceNumber||''}
                </h6>
              </div>
              <div className="text-end ms-4">
                <h6 className="fw-bold mt-1 mb-2">Montant &nbsp;dû:</h6>
  ...
  <div className="fw-bold">Numéro:</div>
  <div>{this.props.info.phone || ''}</div> 
  <div className="fw-bold">Addresse:</div>
  <div>{this.props.info.billToAddress || ''}</div>
</Col>

                
                <Col md={4}>
                  <div className="fw-bold mt-2">''Date de délivrance'':</div>
            ...
            <Row>
              <Col md={6}>
                <Button variant="primary" className="d-block w-100" onClick={GenerateInvoice}>
                  <BiPaperPlane style={{width: '15px', height: '15px', marginTop: '-3px'}} className="me-2"/>Envoyez la facture 
                </Button>
              </Col>
              <Col md={6}>
